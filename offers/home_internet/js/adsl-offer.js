...
const offers = [
    ...
    },
    {
        ...
        badge: "عرض خاص",
        highlight: "العرض هو اتاحة سعة إضافية هدية للعملاء عند الاشتراك على الباقات السنوية",
        details: [], // 👈 تم حذف النصوص الفارغة هنا لتجنب ظهور نقاط خالية
        buttonText: "لتفاصيل اكثر ",
    ...
    },
    {
        ...
        badge: "عرض خاص",
        highlight: "جدّد اشتراكك واستمتع بـ50% سعة إضافية مجانًا لمدة 3 أشهر",
        details: [
...
function displayOffers() {
    ...
    if (!offersContainer || !noOffersMessage) return;
    ...
    offers.forEach(offer => {
        // 👈 تصفية التفاصيل الفارغة والتأكد من وجود تفاصيل لعرض القائمة
        const validDetails = offer.details.filter(detail => detail.trim() !== "");
        const detailsHTML = validDetails.length > 0 
            ? `<div class="offer-details">
                <ul>
                    ${validDetails.map(detail => `<li>${detail}</li>`).join('')}
                </ul>
               </div>` 
            : '';
