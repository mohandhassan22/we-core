...
/* ─── معالجة رابط الاسترجاع (Recovery Link) ───
   لو المشروع شغال بـ PKCE flow، الرابط بييجي فيه ?code=xxxx
   بدل access_token في الـ hash، فلازم نعمل exchange للـ code يدويًا
   قبل ما نستنى onAuthStateChange. */
async function handleRecoveryLink() {
  const params = new URLSearchParams(window.location.search);
  const code = params.get('code');

  if (!code) return; // مفيش code، يبقى غالبًا implicit flow وهيتمسك من onAuthStateChange عادي
  ...
  if (error) {
    ...
    showMsg('الرابط غير صالح أو منتهي الصلاحية.\nالرجاء طلب رابط جديد من صفحة تسجيل الدخول.', 'error');
    return;
  }

  // تنظيف الـ code من الرابط بعد الاستخدام (اختياري لكن مستحسن للأمان)
